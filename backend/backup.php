...
if (!file_exists(__DIR__ . '/backups')) mkdir(__DIR__ . '/backups', 0777, true);

$sqliteBackup = __DIR__ . '/backups/sqlite_backup_' . date('Ymd_His') . '.db';
copy($sqlite_db_path, $sqliteBackup);
...
foreach ($tables as $table) {
    ...
    for ($i = 1; $i <= $ncols; $i++) $header[] = oci_field_name($stmt, $i);
    fputcsv($fp, $header);
    while ($row = oci_fetch_assoc($stmt)) fputcsv($fp, $row);
    fclose($fp);
}
